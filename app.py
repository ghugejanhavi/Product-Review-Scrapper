...
# suppose server is in US and we are accessing the link from India then we have to use flask_cors cross_origin
import requests
...
@cross_origin()
def homepage():
    return render_template('index.html') # on hitting base url it will render the index.html page

# base url + /
#http://localhost:8000 + /
@app.route('/review',methods=['POST']) # when search is clicked this url is hit...view page source for clarity
@cross_origin()
def index():
